...
export function Message({isFromYou, content, audioId}: MessageProps){
    return(
        <div className={`flex items-center ${isFromYou && "justify-end"} p-2 text-sm w-full relative `}>
            ...
            <p className={`max-w-60 ${isFromYou?"bg-orange-800 text-stone-50" :"bg-zinc-200"} p-4 ${!isFromYou && "mt-2"} rounded-xl ${isFromYou? "rounded-br-none": "rounded-bl-none"}`}>{content}</p>
        </div>
    )
}